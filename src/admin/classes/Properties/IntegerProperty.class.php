<?php
	final class IntegerProperty extends BaseProperty
	{
		...
		{
			return '<property name="'.$this->property->getPropertyName().'" type="Integer" required="false" />';
		}

		public function add2form(Form $form)
		{
			return $form->add(Primitive::integer($this->property->getPropertyName()));
		}
		...
		public function add2multiform(Form $form)
		{
			...
			return $form->add(Primitive::integer($primitiveName));
		}

		public function add2search(Form $form)
		{
			return $form->
				add(Primitive::integer($this->property->getPropertyName().'_from'))->
				add(Primitive::integer($this->property->getPropertyName().'_to'));
		}
