#!/usr/bin/php
<?php
	/* $Id: build.php 5514 2008-09-22 11:32:58Z voxus $ */
...
Possible options:
	...
	--check-enumerations:
		check enumeration's reference integrity.
...
<?php
		...
		foreach ($args as $arg) {
			if ($arg[0] == '-') {
				switch ($arg) {
					...
					case '--check-enumerations':
						$metaCheckEnumerations = true;
						break;
	...
	if ($pathMeta && $pathConfig) {
		...
		try {
			...
			if ($metaCheckEnumerations)
				$meta->checkEnumerations(true);
